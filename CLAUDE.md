...
- **When one side of a Host/Expander relationship needs to find the other regardless of physical adjacency, resolve it via a live scan against state the OTHER side already authoritatively holds — never by having this side remember the other's module id and hope it stays valid.** X-family Expanders (X8/X8D/X16/X16D) originally tried "remember the Host's module id via adjacency, fall back to `APP->engine->getModule()` on that remembered id later" for the non-adjacent stay-connected feature — live testing found this genuinely goes stale *within a single session, no reload involved*: a debug readout showed the remembered id and the Host's real current id disagreeing moments after a fresh engage+detach. The fix (`findXBoundHost()`, `XShared.hpp`) has the Expander scan every module in the rack every tick for whichever Host reports `getXParamBoundId(i) == myId` — the exact same authoritative state a Host already maintains for its own purposes (Morpheus's own refresh loop already finds its bound Expander purely by id, `APP->engine->getModule(boundExpanderId)` — Dieter's own diagnosis: "why is there any code handling a disattach from the host's neighborhood anyway" when the Host side already proves this pattern works). A scan against live authoritative state can't go stale because nothing is memorized; a remembered id can, because nothing forces it to refresh. Only fall back to genuine physical adjacency for the case where there's nothing to scan for yet (not yet bound to anything) — see `XHostImplementationGuide.md`'s "Non-adjacent, persistent connection" section for the full writeup, including why disengaging a binding also had to move from an adjacency-only chain-walk to this same id-based lookup.
