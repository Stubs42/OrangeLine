...
**Space in the Windows user name breaks `make`'s own `include` directive, not just env propagation:** this machine's actual Windows account is `Dieter Stubler` (a literal space), so `$HOME`/`setenv.sh`'s own `RACK_DIR=$HOME/RackDevelopment/Rack-SDK` resolves to a path containing a space (`/c/Users/Dieter Stubler/RackDevelopment/Rack-SDK`). GNU Make's `include $(RACK_DIR)/plugin.mk` cannot handle a space in that path *at all* (splits it into two words, "No such file or directory" on the first half) — this is a genuine Make limitation, not fixable via quoting `RACK_DIR` itself (`make RACK_DIR="path with space" -n ...` still fails the same way, since the space breaks Make's own word-splitting inside the `include` line, before quoting from the command line ever comes into play) and separate from the env-propagation quirk in the paragraph above. Workaround: the repo checkout itself is also reachable via a second, space-free path on this machine, `/c/msys64/home/Dieter/RackDevelopment/Rack-SDK` — use *that* as `RACK_DIR` for any direct-`g++`-invocation compile-check (see the workaround above), bypassing `make`'s own `include` resolution entirely rather than trying to fix the space. Confirmed working: build the flags by hand from `$RACK_DIR/plugin.mk`/`compile.mk`/`arch.mk` (`-I$(RACK_DIR)/include -I$(RACK_DIR)/dep/include -fPIC`, plus `compile.mk`'s own `-MMD -MP -g -O3 -funsafe-math-optimizations -fno-omit-frame-pointer -Wall -Wextra -Wno-unused-parameter -std=c++11 -march=nehalem -D_USE_MATH_DEFINES -municode -Wsuggest-override` for a Windows/x64 target) and invoke `g++` directly with the space-free `RACK_DIR` — no `make` call, no space problem, no need to touch `setenv.sh` or the real `RACK_USER_DIR` (which correctly keeps using the spaced path, since Rack itself has no trouble with it at runtime — this only ever bites `make`'s own `include`).
...
- **Display height is barely taller than its own font size** - close to zero top/bottom padding
  ...
  box vertically.
  **Exception: a display packed into a nested, live-resizing sub-block (e.g. NEO's header-widget
  pool - several small knob+display pairs stacked into two rows inside the already-resizable row
  header) should NOT use this font-based formula at all.** Derive its height instead from the
  ENCLOSING frame's own real inner space, using the same "padding is the fixed, primary quantity,
  size is whatever's left over" principle already established for whole rows (see "Resizable
  panel design" below) - just applied one level deeper, to a sub-block nested inside a row instead
  of to the row itself: e.g. a padding/display/padding/display/padding split of the container's
  real inner height, recomputed fresh every `step()` (not a fixed `Neo.hpp` constant) since it
  depends on the live Grid Rows/Full Height state exactly like the container itself does. First
  built for NEO's Range Min/Max/Secondary Track/Channel pool display widgets (2026-07-22) - the
  first-pass version used the font-based formula instead and came out much too tall once actually
  seen live (Dieter's own catch: "the display frames do not follow our formula for display fields
...
- **A numeric readout whose field is sized for its own maximum character budget (not its typical
  content) should right-align, not left-align.** A value like "5" or "0.000" drawn left-aligned
  inside a field sized for "-9.999" hugs the left edge with a ragged, inconsistent right margin -
  wrong for a digital/calculator-style readout, which conventionally right-justifies. `OrangeLine.
  hpp`'s shared `olDrawDisplayText()` takes an `OLDisplayAlign align` parameter (`OL_DISPLAY_ALIGN_
  LEFT`/`RIGHT`/`CENTER`, default LEFT - every existing caller unaffected) for exactly this; RIGHT
  measures its inset from the right edge instead of the left, CENTER ignores the inset entirely
  and anchors on the box's own horizontal center. First built for NEO's Range Min/Max/Secondary
  Channel pool displays (2026-07-22) - started as a plain `rightAlign` bool, then generalized into
  this enum per Dieter's own instruction once a `CENTER` case seemed plausible too ("we will
  exchange the right align bool with an alignment which default ALIGN_LEFT but also allows for
  ALIGN_RIGHT and ALIGN_CENTER"). A field whose formatted content is itself already fixed-width
  (e.g. a zero-padded `"%2d"`-style channel number matching its own field's exact character count)
  doesn't strictly need this - left-align already reads as right-aligned there via the padding
  itself - but set it anyway for any genuinely numeric field, since the fixed-width trick is
  fragile (depends on the font being truly monospaced) and the enum documents intent directly.
...
- **`Module::onRemove()` fires *while* Rack's own `removeModule()` still holds an exclusive lock — any share-locking Rack engine call from within it (or anything it calls, transitively, no matter how many functions deep) is a guaranteed self-deadlock, not a maybe.** Found live: deleting a multi-module selection (a Morpheus + its bound Expanders) froze Rack, reproducible. Root cause: both sides' `onRemove()` handlers (added to proactively clear a bound-Expander/Host reference before the other side is destroyed — the correct fix for a real dangling-pointer risk, see the entry above) called the *ordinary* `APP->engine->getModule()`, documented in `Engine.hpp` as "Share-locks", from inside a callback that Rack's own docs say fires "before removing the module from the Engine" — i.e. during `removeModule()`'s own "Exclusively locks" section. Rack's own rule, stated directly in `Engine.hpp`: "Methods that exclusively lock cannot be called simultaneously or recursively with another share-locking or exclusive-locking method." The fix isn't "call `getModule()` less" — it's using the dedicated `APP->engine->getModule_NoLock()` variant (Rack ships one publicly, exactly for callback contexts that are already inside a lock) for *every* engine lookup anywhere in the call chain triggered from `onRemove()` — including calls made through an interface method (`XHostInterface::resetXParam()` internally calls the locking `getModule()`, so a *new*, separate NoLock variant had to be added, and `onRemove()` must call that one specifically, never the ordinary one). A method that's safe to call from `moduleProcess()` (share-locked via `stepBlock()`, where share+share nesting is documented as fine) is *not* automatically safe to call from `onRemove()` (exclusively locked) — check which lock (if any) a callback already holds before assuming an existing "safe" helper is still safe to reuse there.
- **`Engine::resetModule()` (Right-click "Initialize") holds the EXACT SAME exclusive lock, for its own `onReset()` callback, that `removeModule()` holds for `onRemove()` — the NoLock-variant rule above applies there too, and this was missed the first time round.** Confirmed live via gdb (2026-07-23), reproducible: connect an X8 (or any X-family Expander) to Morpheus, then right-click Morpheus → Initialize → Rack freezes. Root cause: `Morpheus::moduleReset()` (called from the shared `onReset()` in `OrangeLineCommon.hpp`) cleared every X-family candidate binding via the ordinary, share-locking `resetXParam()` — while `Engine::resetModule()` (itself documented "Exclusively locks" in `Engine.hpp`) was still holding the exclusive lock for the entire `onReset()` callback, on the SAME thread. A real stack trace nailed it exactly: the UI thread stuck in `SharedMutex::lock_shared()` inside `Engine::getModule()`, called from `Morpheus::onReset()`, called from `Engine::resetModule()` — a same-thread, non-reentrant self-deadlock, not a race between two threads (a second thread, the audio engine's own `stepBlock()`, was *also* stuck waiting on the same mutex, but only as collateral damage from the writer never releasing). The existing `resetXParamDuringRemoval()` NoLock variant (see the entry above) had already solved this exact problem for `onRemove()` — it just hadn't occurred to anyone that `onReset()`/Initialize needed the identical treatment, since the two call chains look unrelated at a glance. Fixed by routing `moduleReset()` through the same NoLock method, and renaming it from `resetXParamDuringRemoval()` to `resetXParamNoLock()` (`XShared.hpp`) since it's no longer removal-specific — any *future* lifecycle callback that turns out to also hold an exclusive lock should reuse this same NoLock method rather than assuming only `onRemove()` needs it. **General lesson: when a Host/Expander interface has one "safe for use during an exclusive lock" variant of a method, audit *every* lifecycle callback (`onRemove()`, `onReset()`, and any future one) for whether Rack's own engine holds a lock across it, rather than assuming the single already-found case is the only one.**
...
- **Debugging a Rack freeze**: don't speculate about which lock or which call site — attach gdb to the hung process and run **`thread apply all bt`** (all threads, not just the current one — Rack is multi-threaded, and the two threads that matter, the one holding/awaiting the exclusive lock and the one stuck on a nested share lock, are almost never the same one gdb attaches to by default). `detach` + `quit` releases the process without killing it. Both deadlocks in the entries above were only actually confirmed, not just guessed at, this way — a real stack trace showing two engine calls both mid-lock is unambiguous in a way static code reading isn't. `tools/rack_stacktrace.sh` (2026-07-23) automates the whole sequence — run it from a real interactive MSYS2 MinGW64 terminal, **never** from a sandboxed/agent shell (confirmed: gdb attach fails there even against a process that same sandboxed shell itself spawned, `error 5`/Access Denied — a genuine sandbox permission wall, not a real-terminal issue). **`ps -W`'s column 1 is the MSYS-internal PID, NOT what gdb needs** — gdb (a native Windows debugger) requires the real Windows PID, which `ps -W` reports separately as `WINPID` in column 4; passing column 1 to `gdb -p` produces a *different*-looking but equally uninformative failure, `error 87: The parameter is incorrect` (the script already gets this right). Also worth checking before assuming a pure deadlock: is `WerFault.exe` running with `/p <that same WINPID>` in its command line (`Get-CimInstance Win32_Process -Filter "Name='WerFault.exe'"` via `powershell.exe`)? If so, Windows itself has already flagged the process as crashed/unresponsive and is mid-crash-handshake, which *also* blocks a fresh gdb attach until WerFault is dealt with (killed, or let finish) — a real, confirmed cause of "gdb won't attach," not just a deadlock-specific quirk.
- **Every entry in a module's own `jsonIds` enum needs a real `setJsonLabel()` call, even one that's genuinely never read or written** — `OrangeLineCommon.hpp`'s `dataToJson()` does `json_object_set_new(rootJ, OL_jsonLabel[jsonIdx], ...)` unconditionally for every slot up to `NUM_JSONS`, with no null-check, and `OL_jsonLabel[]` is never zero-initialized anywhere. A slot that never gets `setJsonLabel()` called on it is live, uninitialized memory, not a harmless no-op — passing that garbage pointer to jansson as a JSON key crashes the instant any instance is first serialized (module-browser creation, undo-history snapshot, autosave). Found twice in the same session on NEO: once from deliberately leaving a since-retired field's slot "unused" without realizing the label itself was still load-bearing, and once as a pre-existing, unrelated omission on a field that WAS actively read. If a field is genuinely retired, follow the established pattern elsewhere in this codebase (see `CONNECTED_HOST_ID_JSON`/`CHANNEL_COLOR_JSON`/`ROW_MEMTAPE_JSON`'s own comments) — either delete the enum entry outright (accepting renumbering, already a precedent for a deliberate breaking change pre-release) or keep the slot **and its label** as a permanently-unwritten dead field. Never leave a slot in the enum with no matching `setJsonLabel()` call. After any such change, grep every `_JSON` enum entry against every `setJsonLabel()` call to confirm full coverage — don't just check the one slot just touched.
- **Rack's own `clamp(x, a, b)` (`math.hpp`) explicitly documents: "If `b < a`, returns `a`"** — i.e. if the upper bound ends up smaller than the lower bound, the call unconditionally returns the LOWER bound regardless of `x`. Two independently-computed bounds that are each individually correct in isolation can still collide this way once a live, content-dependent value swings one of them — NEO's own drag-resize clamp computes a floor from a fixed "always show ≥4 columns" invariant and a ceiling from a connected Host's own real (possibly short) channel content; the moment the Host's longest channel was shorter than 4 steps, the ceiling dropped below the floor and every resize attempt silently snapped to the exact same fixed width regardless of mouse movement, with no error anywhere. Symptom is easy to misdiagnose as "the control stopped responding" rather than "the bounds passed to clamp() are inverted" — whenever two bounds for the same `clamp()` call are computed via genuinely different formulas/data sources (a structural invariant vs. live content), explicitly floor the upper bound to the lower one (`upper = std::max(upper, lower)`) at the point they're used together, rather than trusting they'll always agree. **The same trap resurfaces when clamping a PAIR of user-configured bounds (min AND max) against one live source range** — NEO's own shrink-only range feature (`NeoValueCellEditorBase::computeEffectiveRange()`) clamps a user's own `rangeMax` against `[rangeMin, sourceMax]`, not against `[sourceMin, sourceMax]` directly: clamping it against the ORIGINAL `sourceMin` first, then separately checking it against the already-clamped `outMin`, would let the same b<a inversion sneak in the moment the user's raw max sits below the user's own (already-clamped) min. The fix is the same shape as the floor-to-lower-bound rule above, just applied one step earlier in the chain: compute `outMin` first, then clamp `outMax` against `outMin` (not the original lower bound) as ITS OWN lower argument — this guarantees `outMax >= outMin` always, collapsing to a single degenerate point in the worst case rather than ever inverting.
- **A per-frame UI-widget `step()` that resyncs local editable state (e.g. a `ui::TextField`'s `text`/`cursor`) from a module-side cache populated by the DSP thread will race against that same widget's own in-flight local edit, and `std::min()`-style clamping after the resync can silently stick a derived value below its true value forever.** NEO's `NeoRowNameField::step()` re-read `channelName[]` (only refreshed by `refreshChannelTable()` on `moduleProcess()`, a separate thread) every UI frame to catch edits made by *other* NEO instances — but the very next `step()` after a local keystroke could see a still-stale, one-character-shorter cached value before the DSP thread caught up, reset `text` back to it, and `cursor = std::min(cursor, text.size())` clamped the cursor down to match. Once the DSP thread caught up a moment later and `text` corrected itself again, the cursor never grew back — `std::min()` only ever shrinks — so it stayed stranded exactly one position behind the true end of the string, permanently, for the rest of that edit. Symptom: appending a character always left the cursor sitting before the just-typed character instead of advancing past it, a very specific and easy-to-underestimate-as-cosmetic bug. Fix: gate the resync so it's skipped entirely while the widget currently has focus (`APP->event->getSelectedWidget() == this`) — this same field's own in-flight edit is authoritative while focused, and the external-resync purpose only applies once focus is elsewhere.
- **A custom `ParamQuantity` subclass that needs to call a free function defined LATER in the same file can't just be defined inline where it's first needed, if that spot is itself inside a module struct's own inline member function body (e.g. `moduleParamConfig()`).** `configParam<T>()` requires `T` to be a *complete* type by the point the enclosing struct (`struct Neo { ... }`) itself closes, because of C++'s "complete-class context" rule for deferred-parsed inline member function bodies (every module struct in this codebase is defined this way, per the `#include`-composition architecture) — but the `ParamQuantity` subclass's own real body (`NeoSecondaryTrackParamQuantity::getDisplayValueString()`, needing `neoCellEditorForRow()`, itself defined only much later in the file) can't be written yet at that point. Resolved (2026-07-22) with a standard, always-legal C++ technique that isn't otherwise used elsewhere in this codebase: declare the class early with only a method PROTOTYPE (`std::string getDisplayValueString() override;`, no body) — satisfying the completeness requirement — then define the actual out-of-line body (`ReturnType ClassName::methodName() { ... }`) much later in the file, right after the free function it needs becomes available. Avoids needing a `neoCellEditorRegistry()`-style forwarding free-function just to break the ordering dependency. Reach for this same shell-declare-then-define-later shape whenever a `ParamQuantity`/similar helper class needs to be usable from inside an inline-defined module struct but its real logic depends on something declared further down the same file.
