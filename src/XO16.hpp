...
*/
// Shared verbatim by XOD16.cpp too (identical enum shape - XOD16 just adds a display column on
// top in its own widget, same "X16D reuses X16.hpp" economy CLAUDE.md documents). Jack-bearing
// member of the XO family - relays a real Host signal into a re-emitted output jack at
// control-rate throttle, exactly the latency problem Wakeup/Ready exists for - gets full Touch.
...
//
// Input Ids - XO16 has no real CV inputs at all
//
enum InputIds {
	NUM_INPUTS
};
