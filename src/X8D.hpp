...
*/
// X8D has no real CV jacks at all (it's a param-access Expander, no sockets by design - see
...
enum jsonIds {
	STYLE_JSON,
	CHANNEL_LIMIT_JSON,
	BROWSE_INDEX_JSON,  // persists the last browsed param - see X8D.cpp's moduleProcess()
...
// Parameter Ids
//
#define NUM_X8D_KNOBS 8
enum ParamIds {
	KNOB_PARAM,
	KNOB_PARAM_LAST = KNOB_PARAM + NUM_X8D_KNOBS - 1,
	LEFT_PARAM,
	RIGHT_PARAM,
	ENGAGE_PARAM,
...
//
// Input Ids - X8D has no real CV inputs at all
//
enum InputIds {
	NUM_INPUTS
};
...
//
// Output Ids - X8D has no real CV outputs at all
//
enum OutputIds {
	NUM_OUTPUTS
};
...
enum LightIds {
	...
	CONN_LIGHT_LAST = CONN_LIGHT + 1, // GreenRedLight needs 2 consecutive ids (green, red) - see
	                                  // the host type-lock feature in X8D.cpp
